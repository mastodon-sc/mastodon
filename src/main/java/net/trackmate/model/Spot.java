...
public class Spot< V extends Spot< V >> extends AbstractVertex< V, Link< V >, ByteMappedElement > implements RealLocalizable, HasTimepoint
{
	...
	Spot< V > init( final int timepointId, final double x, final double y, final double z, final double radius )
	{
		setX( x );
		setY( y );
		setZ( z );
	...
	Spot( final AbstractVertexPool< V, Link< V >, ByteMappedElement > pool )
	{
		super( pool );
	}
