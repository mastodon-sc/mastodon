...
import org.mastodon.revised.model.feature.FeatureModel.FeatureModelListener;
		...
		ContextListener< V >
{
			...
			final UndoPointMarker undoPointMarker )
	{
		...
		final Class< ? > vertexClass = viewGraph.vertexRef().getClass();
		final Set< Feature< ?, ? > > vertexFeatures = featureModel.getFeatureSet( vertexClass );
		...
		final Class< ? > edgeClass = viewGraph.edgeRef().getClass();
		final Set< Feature< ?, ? > > edgeFeatures = featureModel.getFeatureSet( edgeClass );
	...
	{
		ignoreTableSelectionChange = true;
		final Set< Feature< ?, ? > > vertexFeatures = featureModel.getFeatureSet( vref.getClass() );
		vertexTable.setFeatures( vertexFeatures );
		final Set< Feature< ?, ? > > edgeFeatures = featureModel.getFeatureSet( eref.getClass() );
		edgeTable.setFeatures( edgeFeatures );
		if ( mirrorSelection )
			selectionChanged();
		ignoreTableSelectionChange = false;
	...
	public void contextChanged( final Context< V > context )
	{
		...
		if ( null == context )
		{
			vertexTable.setFilterBy( null );
			edgeTable.setFilterBy( null );
		}
		else
		{
			...
			vertexTable.setFilterBy( filterByVertices );
			edgeTable.setFilterBy( filterByEdges );
		}
		if ( mirrorSelection )
			selectionChanged();
		ignoreTableSelectionChange = false;
